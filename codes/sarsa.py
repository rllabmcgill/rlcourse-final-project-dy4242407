...
# the following code defines the environment
class Summarization():
...
# the following code parametrize the state(summary) and output the feature
class Features():
...
class sarsa_e_trace:
    def __init__(self, environment, features, mlambda_theta=0.5, gamma=1, alpha=0.01, tau=0.987):
    ...
    def cal_V(self, s):
        return np.dot(self.features.phi(s),self.theta)
    

    def cal_Q(self, s, a):
        env1 = copy.deepcopy(self.env)
        s_next, term, r = env1._step(a)
        return r + self.gamma * self.cal_V(s_next)
        
    def policy_prob(self, s, a, action_set, tau):
        
        pref_v = [self.cal_Q(s,a) for a in action_set]
        
        m = np.max(pref_v)
        e_sum = np.sum(np.exp((pref_v - m)/tau))
        pi = np.exp((self.cal_Q(s,a)-m)/tau) / e_sum
                   
        return  pi
        
    # this function returns an action, where the action a is chosen with prob pi(a|s,theta)
    # using policy parametrization
    def policy_par(self, state, action_set, tau):
        
        pi_actions = []
        
        for a in action_set:
            pi_actions.append(self.policy_prob(state, a, action_set, tau))
                
        return np.random.choice(action_set, p= pi_actions)
    
    
    def policy_greedy(self, state, action_set, tau):
    ...
    def greedy_select(self):
        ...
        while not term:
            ...
            if len(s)<self.env.length_limit:
                    a = self.policy_greedy(s, action_set, self.tau)
    ...
    def train(self, iterations):        
        ...
        for episode in range(iterations):
            ...
            while not term:
                ...
                if len(s)<self.env.length_limit:
                    a = self.policy_par(s, action_set, np.power(self.tau, time_step))
                #cheat a bit here, force to stop before going over the length limit
                else:
                    a=0
                ...
                e_theta = self.gamma * self.mlambda_theta * e_theta + self.features.phi(s)
                
                # learning with current learning rate
                alpha = self.alpha*101/(100 + np.power(time_step+1, 1.1))
                self.theta += alpha * delta * e_theta
            
                s = s_next
                
                time_step += 1 
                #print("time_step",time_step, "action",a)
        ...
        self.greedy_summary = final_summary
...
t=time()
agent = sarsa_e_trace(env, features)
...
#%%
def average_run(env,features, num_runs, episodes_per_run, mlambda_theta=0.5):
    
    m = np.zeros((num_runs, episodes_per_run))
    
    for i in range (num_runs):
        
        agent = sarsa_e_trace(env, features, mlambda_theta)
...
#    avg_returns = average_run(env, features, 3, 300, i)
#    reinforce_mean = runningMean(avg_returns, N=10)
#    plt.plot(reinforce_mean, label='sarsa, lambda=%s'%i)
#plt.xlabel('Episodes')
